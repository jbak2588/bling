...
class _AiAuditScreenState extends State<AiAuditScreen>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('ai_audit.title'.tr()),
        bottom: TabBar(
          controller: _tabController,
          tabs: [
            Tab(text: 'ai_audit.tabs.all'.tr()),
            Tab(text: 'ai_audit.tabs.takeover'.tr()),
            Tab(text: 'ai_audit.tabs.registration'.tr()),
          ],
        ),
      ),
      body: TabBarView(
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          return Center(
              child: Text("${'ai_audit.list.error'.tr()}: ${snapshot.error}"));
        }
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return Center(child: Text('ai_audit.list.no_records'.tr()));
        }
  ...
  Widget _buildCaseTile(AiCaseModel item) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(
          item.stage == 'takeover'
              ? 'ai_audit.list.takeover_title'.tr()
              : 'ai_audit.list.registration_title'.tr(),
        ...
        subtitle: Column(
          ...
          children: [
            ...
            Text("${'ai_audit.list.verdict'.tr()}: ${item.verdict ?? 'N/A'}"),
            Text(
              "${'ai_audit.list.id'.tr()}: ${item.caseId.substring(0, 8)}... • ${timeago.format(item.createdAt.toDate())}",
