...
class _AdminProductDetailScreenState extends State<AdminProductDetailScreen> {
  ...
  Future<void> _rejectProduct() async {
    ...
    setState(() => _isLoading = true);
    await _repository.updateProductStatus(
        productId: widget.product.id, status: 'rejected', rejectionReason: '관리자 거절');
    if(mounted) Navigator.pop(context);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('검수 요청 상세')),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(widget.product.title, style: Theme.of(context).textTheme.headlineSmall),
